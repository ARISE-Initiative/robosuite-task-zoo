...
from robosuite_task_zoo.models.kitchen_gck import PotObject, StoveObject, ButtonObject, CabinetObject, ServingRegionObject, MicrowaveObject, CupObject, SpatulaObject, BottleObject

# from robosuite_task_zoo.models.multitask_kitchen import 

class KitchenGckDomain(SingleArmEnv):
    """
    KitchenGckDomain: A multi-task environment with different varaints (different initial configurations).
    """
    def __init__(
        self,
        robots,
        ...
        self.table_full_size = (2.0, 2.0, 0.05)
    ...
    def _load_model(self):
        ...
        # gck added objects
        self.microwave_object = MicrowaveObject(
            name="MicrowaveObject")
        microwave_object = self.microwave_object.get_obj(); microwave_object.set("quat", array_to_string((0., 0., 0., 1.))); microwave_object.set("pos", array_to_string((0.5, -0.7, 0.03))); mujoco_arena.table_body.append(microwave_object)

        self.cup_object = CupObject(
            name="CupObject")
        cup_object = self.cup_object.get_obj(); cup_object.set("quat", array_to_string((0., 0., 0., 1.))); cup_object.set("pos", array_to_string((0.5, 0.8, 0.03))); mujoco_arena.table_body.append(cup_object)

        self.spatula_object = SpatulaObject(
            name="SpatulaObject")
        spatula_object = self.spatula_object.get_obj(); spatula_object.set("quat", array_to_string((0., 0., 0., 1.))); spatula_object.set("pos", array_to_string((-0.2, 0.8, 0.03))); mujoco_arena.table_body.append(spatula_object)

        self.bottle_object = BottleObject(
            name="BottleObject")
        bottle_object = self.bottle_object.get_obj(); bottle_object.set("quat", array_to_string((0., 0., 0., 1.))); bottle_object.set("pos", array_to_string((0.65, 0.8, 0.03))); mujoco_arena.table_body.append(bottle_object)
        ...
        # gck added
        self.model.merge_assets(self.microwave_object)
        self.model.merge_assets(self.cup_object)
        self.model.merge_assets(self.spatula_object)
        self.model.merge_assets(self.bottle_object)
